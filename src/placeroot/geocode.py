...
def _alt_rows_not_already_found(
    ...
    """
    try:
        rows = _query_alt_names(alt_table, table_path, query, region_code, near=near)
    ...
    if local_table is not None:
        ...
        try:
            rows = _query_divisions_from_local(
                local_table, query, region_code, name_expr, near=near
            )
        except overture.UpstreamUnavailable:
            ...
            if alt_table is not None:
                rows = rows + _alt_rows_not_already_found(
                    alt_table, local_table, query, region_code, rows, near=near
                )
            return rows
    name_expr = "strip_accents(names.primary)" if fold_diacritics else "names.primary"
    return _query_divisions_from_upstream(query, region_code, name_expr, near=near)
...
def geocode(
    ...
    return geocode_detailed(query, limit, lang=lang, near=near)["results"]
...
def geocode_detailed(
    ...
    *,
    ...
    divisions = _query_divisions(
        search_query, region_code, local_table, alt_table=alt_table, **near_kw
    )
    ...
    if not literal_answer_is_good_enough:
        for variant_query in _abbreviation_variant_queries(search_query):
            for row in _query_divisions(variant_query, region_code, local_table, **near_kw):
    ...
    if local_table is not None or not literal_answer_is_good_enough:
        ...
        rows = (
            _query_divisions(
                stripped_query, region_code, local_table, fold_diacritics=True, **near_kw
            )
            if stripped_query
            else []
        )
    ...
    if not divisions and local_table is not None and not _has_like_metacharacter(fuzzy_query):
        if _is_bundled_table(local_table):
            ...
            fuzzy_rows = _query_divisions_fuzzy(
                local_table, fuzzy_query, suffix_region_code, **near_kw
            )
    ...
    if not candidates and _bundled_recall_pending and not _names_a_feature(search_query):
        ...
        recalled = _query_divisions(search_query, region_code, None, **near_kw)
...
def resolve_place(
    ...
    if city_bounded and near_lat is not None and near_lon is not None:
        ...
        geocode_hits = geocode(
            ...
            near=(near_lat, near_lon, _CITY_HINT_RADIUS_M),
        )
        geocode_hits = [
