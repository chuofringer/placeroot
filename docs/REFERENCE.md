...
| Tool | Answers |
|---|---|
| `find_places` | Named places near a point (`lat`/`lon`, or `where` — a `{lat,lon}` dict, GERS id, or free-text place name, resolved server-side with a compact `resolved` echo) **or inside a named area / division polygon**, nearest first — filter by category (or up to 5 `categories` in one scan, optionally `group_by_category`), brand, confidence, operating status, or has-website / has-phone; `detail` picks the row shape (`compact`, the default: id/name/category/lat/lon/distance_m/trust tier; `ids`: id/distance_m only, no coordinates; `full`: every field incl. trust_note prose); a `name` filter with no literal match falls back to an alt-spelling then a typo-tolerant fuzzy match, tagging the row and noting the correction |
...
| `suggest_areas` | The inverse of every other area tool: 1–3 anchor points + travel budgets (walk/cycle/drive, per-anchor minutes) + 1–8 free-text amenity requirements → a ranked shortlist of reachable neighborhoods/localities, each with per-anchor travel times, per-requirement scores, and a one-line reason |
...
| `geocode_intersection` | Locate where two named streets cross: "5th Avenue", "Main Street", "Portland" — city-bounded, walk-graph-extracted, nearest-to-center crossings capped at 5 |
...
| `resolve_map_url` | A pasted Google/Apple/OpenStreetMap/`geo:` map link → its coordinate, zoom and place name, plus a reverse-geocoded `place` row — parsed offline, no API key; only a Google short link (`maps.app.goo.gl`) is followed over the network; a link that only names a place resolves by name |
| `compare_modes` | Walk vs cycle vs drive between two places in one call, instead of three `route` calls: the ends (`from`/`to`, same forms as `from_to`) resolve once, every requested mode routes between the same coordinates, and each mode gets one compact row (`distance_m`, `duration_s`, `duration_min`, optional climb/descent with `include_elevation=true`) — no path, no export. `fastest`/`shortest` name the winning modes and `summary` is a deterministic sentence; a mode that cannot be routed (`too_far`, `needs_confirm`, `no_route`, ...) fails inline in its own row rather than aborting the call. Drive time is a posted-speed model with no live traffic |
...
| `neighborhood_verdict` | Should I live here? A ranked verdict from life context (household, mobility, priorities) — strengths, weak points, one thing to verify in person; transit also reads base-theme stops (#454), not just places |
...
title, and `readOnlyHint: true` on the 40 that are pure lookups — so a client
...
All 48 tool schemas cost roughly **41.3k tokens** of every conversation's
...
|---|---:|---:|---:|
| unset / `all` (default) | 48 | ~41,293 | — |
| `search` | 14 | ~14,213 | 66% |
| `core` | 15 | ~17,976 | 56% |
| `routing` | 11 | ~13,599 | 67% |
| `analysis` | 14 | ~10,900 | 74% |
| `geometry` | 3 | ~3,355 | 91% |
| `progressive` | 4 (all 48 reachable) | ~1,406 | 97% |
...
- **`search`** — the find/name/identify family: `find_places`, `find_near`, `place_details`, `geocode`, `resolve_place`, `reverse_geocode`, their `*_batch` siblings, `address_at`, `geocode_address`, `geocode_intersection`, `search_categories`, and `gers_lookup`.
- **`routing`** — `route`, `from_to`, `isochrone`, `distance_matrix`, `travel_time_matrix`, `within_distance`, `optimize_route`, `elevation_at`, `meeting_point`, `map_match`, `compare_modes`.
- **`analysis`** — `summarize_area`, `summarize_buildings`, `compare_areas`, `suggest_areas`, `buildings_at`, `land_use_at`, `infrastructure_at`, `transit_stops_near`, `water_near`, `admin_lookup`, `timezone_at`, `neighborhood_verdict`, `changes_in_area`, `verify_claims`.
- **`geometry`** — `simplify_geometry`, `render_map`, `geometry_op`.
- **`progressive`** — not a slice of the surface but a door to it: `placeroot_capabilities()` returns a ~1,000-token catalog of all 48 tools (name, one-liner, argument list), and `placeroot_call(tool, args)` runs any of them and returns the tool's own answer unchanged. For the install that wants everything available without paying 41.3k tokens for it in every conversation — profiles need you to know up front which tools you want; this doesn't. One extra round trip when the agent needs the catalog. It replaces the surface rather than adding to it, so it has to stand alone: `PLACEROOT_TOOLS=progressive,core` fails at startup rather than registering both.
...
(`registered 15 of 48 tools (PLACEROOT_TOOLS=core)`), so a selection that
