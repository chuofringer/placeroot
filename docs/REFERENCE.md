...
| Tool | Answers |
|---|---|
| `find_places` | Named places near a point **or inside a named area / division polygon**, nearest first — filter by category (or up to 5 `categories` in one scan, optionally `group_by_category`), brand, confidence, operating status, or has-website / has-phone; `detail` picks the row shape (`compact`, the default: id/name/category/distance_m/trust tier; `ids`: id/distance_m; `full`: every field incl. trust_note prose); a `name` filter with no literal match falls back to an alt-spelling then a typo-tolerant fuzzy match, tagging the row and noting the correction |
...
for a day instead of re-reading a ~25.8k-token schema surface every session.
...
All 42 tool schemas cost roughly **25.8k tokens** of every conversation's
